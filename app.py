...
if menu == "追加":
    ...
    status = st.selectbox("読書ステータス", ["未読", "読書中", "読了", "再読したい", "引用予定"])
            ...
            with st.container():
                ...
                with st.expander("編集"):
                    edit_status = st.selectbox(
                         "読書ステータス",
                         ["未読", "読書中", "読了", "再読したい", "引用予定"],
                        index=["未読", "読書中", "読了", "再読したい", "引用予定"].index(row["status"])
                        if row.get("status") in ["未読", "読書中", "読了", "再読したい", "引用予定"]
                        else 0,
